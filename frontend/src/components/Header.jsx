...
const Header = () => {
  const { user, setUser, collapsed, setCollapsed } = useContext(UserDataContext)
  ...
  return (
    <div className="header">
      <div className={`header-left transition`} style={{ position: 'relative', left: collapsed ? "-16px" : "0px" }}>
        {
          collapsed
            ?
            <a className="logo">
      ...
      <div className="menu-toggle" style={{ cursor: 'pointer', position: 'relative', left: collapsed ? "-174px" : "0px", transition: "all 0.3s ease" }}>
        <a id="toggle_btn" onClick={() => setCollapsed(!collapsed)}>
      ...
      <ul className="nav user-menu">
        <li className="nav-item dropdown noti-dropdown language-drop me-2">
          <a className="dropdown-toggle nav-link header-nav-list" data-bs-toggle="dropdown">
            <img src="/assets/img/icons/header-icon-india.png" alt="" width={35} />
          </a>
        ...
        <li className="nav-item zoom-screen me-2">
          <a className="nav-link header-nav-list win-maximize" style={{ cursor: 'pointer' }} onClick={(e) => {
            e.preventDefault();
            toggleFullScreen();
          }}>
        ...
        <li className="nav-item dropdown has-arrow new-user-menus" ref={menuRef}>
          <a className="dropdown-toggle nav-link" style={{ cursor: 'pointer' }} data-bs-toggle="dropdown" onClick={(e) => {
            e.preventDefault();
            setOpen(!open);
          }}>
          ...
          <div className={`dropdown-menu ${open ? "show" : ""}`} style={{
            opacity: open ? 1 : 0,
